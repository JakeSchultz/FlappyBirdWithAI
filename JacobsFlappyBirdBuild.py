...
SEAN_BIRD = [pygame.transform.scale2x(pygame.image.load(os.path.join("imgs", "sean_bird1.png"))), pygame.transform.scale2x(pygame.image.load(os.path.join("imgs", "sean_bird2.png"))), pygame.transform.scale2x(pygame.image.load(os.path.join("imgs", "sean_bird3.png")))]
OZZY_DOG = [pygame.image.load(os.path.join("imgs", "Ozzy1.png")), pygame.image.load(os.path.join("imgs", "Ozzy2.png")), pygame.image.load(os.path.join("imgs", "Ozzy3.png"))]
...
JUMP_EFFECT = pygame.mixer.Sound(os.path.join("music", "jump.mp3"))
JUMP_EFFECT.set_volume(0.03)

DEATH_EFFECT = pygame.mixer.Sound(os.path.join("music", "death.wav"))
DEATH_EFFECT.set_volume(0.10)

POINT_EFFECT = pygame.mixer.Sound(os.path.join("music", "point.wav"))
POINT_EFFECT.set_volume(0.15)

# Background Music List Will be selected after choosing game mode
MUSIC_LIST = ["Free Synthwave Loop.wav", "Duel_of_the_Fates.mp3", "Guile Theme.mp3"]

BG_MUSIC = pygame.mixer.music
# LOAD TITLE MUSIC
TITLE_MUSIC = (os.path.join("music", "OneFinalEffort-Halo3.mp3"))

# music control flags
_current_track = TITLE_MUSIC
_next_song = TITLE_MUSIC   
_song_selected = -1

def PlayNextSong():
    global _current_track, _next_song
    _current_track = _next_song
    BG_MUSIC.load(_next_song)
    BG_MUSIC.play(-1)

def SelectNextSong():
    global _current_track, _next_song, MUSIC_LIST, TITLE_MUSIC #globals that will be used
    if (_current_track == TITLE_MUSIC and _song_selected == -1):
       _next_song = os.path.join("music", random.choice(MUSIC_LIST))
       PlayNextSong()
    elif (_current_track == TITLE_MUSIC and _song_selected == 0):
        _next_song = os.path.join("music", MUSIC_LIST[0])
        PlayNextSong()
    elif (_current_track == TITLE_MUSIC and _song_selected == 1):
        _next_song = os.path.join("music", MUSIC_LIST[1])
        PlayNextSong()
    elif (_current_track == TITLE_MUSIC and _song_selected == 2):
        _next_song = os.path.join("music", MUSIC_LIST[2])
        PlayNextSong()
    _next_song = TITLE_MUSIC
...
def run_best_AI():
    ...
    BG_MUSIC.fadeout(200)
    #play music file on infinite loop
    SelectNextSong()
    while run:
        clock.tick(30) # means we do 30 ticks per second at max 
        for event in pygame.event.get(): # just keeps track of user input
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: 
                if exit_button.isOver(pos):
                    BG_MUSIC.fadeout(200) #fade music to a stop after 0.2 s 
                    PlayNextSong()
                    run = False
        ...
        for pipe in pipes: 
            if pipe.collide(bird, win): 
                run = False
                BG_MUSIC.fadeout(200) #fade music to a stop after 0.2 s 
                duration = int(DEATH_EFFECT.get_length())*1000
                DEATH_EFFECT.play()
...
def run_normal_mode():
    ...
    run = True
    BG_MUSIC.fadeout(200)
    SelectNextSong() #play next song on an infinite loop and prep for TitleMusic on exit
    while run:
        clock.tick(30) # means we do 30 ticks per second at max 
        for event in pygame.event.get(): # just keeps track of user input
            if event.type == pygame.QUIT:
...
def draw_main_gui(*argv):
    ...
    title = FLAPPY_BIRD_FONT.render(TITLE, 1, (0,0,0))
...
def run_settings_gui():
    ...
    sean_button = Button((197, 235, 207), 25, buttonX, buttonY + 4 * button_gap, button_width, 60, "Sean's Skin")
    ozzy_button = Button((197, 235, 207), 25, buttonX, buttonY + 5 * button_gap, button_width, 60, "Ozzy's Skin")

    normal_button.draw(win)
    alvin_button.draw(win)
    gregory_button.draw(win)
    lillian_button.draw(win)
    sean_button.draw(win)
    ozzy_button.draw(win)
        ...
        for event in pygame.event.get(): 
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: 
                ...
                if lillian_button.isOver(pos): 
                    ...
                    run = False
                if sean_button.isOver(pos):
                    BIRD_IMGS = SEAN_BIRD
                    run = False
                if ozzy_button.isOver(pos):
                    BIRD_IMGS = OZZY_DOG
                    run = False

# runs the music selection gui
def run_music_gui():
    global _song_selected # we will be updating the song selected flag
    ...
    # create title with flappy bird font
    title = FLAPPY_BIRD_FONT.render("Music", 1, (0,0,0))
    ...
    random_button = Button((197, 235, 207), 25, buttonX, buttonY, button_width, 60, "Random")
    song0_button = Button((197, 235, 207), 25, buttonX, buttonY + button_gap, button_width, 60, "Synthwave")
    song1_button = Button((197, 235, 207), 25, buttonX, buttonY + 2 * button_gap, button_width, 60, "Epic")
    song2_button = Button((197, 235, 207), 25, buttonX, buttonY + 3 * button_gap, button_width, 60, "Guile")

    random_button.draw(win)
    song0_button.draw(win)
    song1_button.draw(win)
    song2_button.draw(win)
        ...
        for event in pygame.event.get(): 
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: 
                if random_button.isOver(pos):
                    _song_selected = -1
                    run = False
                if song0_button.isOver(pos): 
                    _song_selected = 0
                    run = False
                if song1_button.isOver(pos): 
                    _song_selected = 1
                    run = False
                if song2_button.isOver(pos):
                    _song_selected = 2
                    run = False
...
def run_gui(): 
    ...
    music_button = Button((197, 235, 207), 25, centerX - (button_width/2), buttonY + 4 * button_gap, button_width, 60, "Music")
    ...
    while run: 
        ...
        draw_main_gui(bird, noAI_button, trainAI_button, bestAI_button, settings_button, music_button)
